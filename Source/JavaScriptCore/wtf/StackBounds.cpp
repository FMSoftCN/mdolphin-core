...
size_t pthread_getstack_np (pthread_t id, void **basestack)
{
    /* Stack size limit.  */
    struct rlimit rl; 
    int ret=0;
    size_t stackSizetemp;
    int *stacksize = (int *)&stackSizetemp;
    void *tempstackaddr;
        ...
        while (! feof_unlocked (fp))
        {
                    ...
                    && (uintptr_t) __libc_stack_end < to) 
            {
                ...
                *stacksize = rl.rlim_cur;
                ...
                tempstackaddr = (void *) from;
#else
                tempstackaddr = (void *) to; 
                /* The limit might be too high.  */
                if ((size_t) *stacksize > (size_t) tempstackaddr - last_to) 
                    *stacksize = (size_t) tempstackaddr - last_to;
#endif
    ...
    tempstackaddr = (void *) ((int)tempstackaddr - *stacksize);
    *basestack = tempstackaddr;
    return *stacksize;
}
...
#endif
#if ENABLE(NO_NPTL)
    int rc = stackSize = pthread_getstack_np(thread, &stackBase);
#else
