/*
** $Id: UidnaMg.c 123 2010-06-22 09:55:14Z xwyan $
**
** UidnaMg.c: implement file for uidna.
**
...
static inline char asciiCaseMap(char b, char uppercase) 
{
    if(uppercase) 
    {
 ...
 */
static inline char digitToBasic(int32_t digit, char uppercase) 
{
...
static inline char isLDHChar(UChar ch)
{
    // high runner case
    if(ch>0x007A)
    {
...
inline static char startsWithPrefix(const UChar* src , int srcLength)
{
 ...
 * @stable ICU 2.8
 */
static inline char isLabelSeparator(UChar ch)
{
    switch(ch)
    {
        case 0x002e:
...
// the separtor.
static inline int getNextSeparator(UChar *src,int srcLength,UChar **limit, char *done)
{
    if(srcLength == -1)
    {
        int i;
                   ...
                   UErrorCode *status)
{
    ...
    int remainingLen = srcLength;
    int remainingDestCapacity = destCapacity;
