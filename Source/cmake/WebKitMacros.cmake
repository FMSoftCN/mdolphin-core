...
MACRO (WEBKIT_INCLUDE_DIRECTORIES)
    LIST (APPEND mDolphin_INCLUDE_DIRECTORIES ${ARGV})
    SET (mDolphin_INCLUDE_DIRECTORIES ${mDolphin_INCLUDE_DIRECTORIES} PARENT_SCOPE)
ENDMACRO ()

MACRO(WEBKIT_APPEND_SOURCES)
    LIST (APPEND mDolphin_SOURCES ${ARGV})
    SET (mDolphin_SOURCES ${mDolphin_SOURCES} PARENT_SCOPE)
ENDMACRO(WEBKIT_APPEND_SOURCES)

MACRO (WEBKIT_ADD_SUBDIRECTORY)
    SET (mDolphin_PARENT_SOURCES ${mDolphin_SOURCES})

    FOREACH (_i ${ARGV})
        SET (mDolphin_SOURCES "")

        ADD_SUBDIRECTORY (${_i})

        IF (mDolphin_SOURCES)
            FOREACH (_file ${mDolphin_SOURCES})
                IF (NOT _file MATCHES "${DERIVED_SOURCES_DIR}")
                    LIST (APPEND mDolphin_PARENT_SOURCES "${_i}/${_file}")
                ELSE ()
                    GET_FILENAME_COMPONENT(_name ${_file} REALPATH)
                    LIST (APPEND mDolphin_PARENT_SOURCES "${_name}")
                ENDIF ()
            ENDFOREACH (_file)
        ENDIF (mDolphin_SOURCES)
    ENDFOREACH (_i ${ARGV})

    SET (mDolphin_SOURCES ${mDolphin_PARENT_SOURCES})
    SET (mDolphin_SOURCES ${mDolphin_SOURCES} PARENT_SCOPE)
    UNSET(mDolphin_PARENT_SOURCES)
ENDMACRO ()
