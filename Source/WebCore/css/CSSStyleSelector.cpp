...
#if PLATFORM(MG) && ENABLE(SPIDER)
    KURL kurl = m_document->completeURL(*attr);
    if (kurl.isValid()) {
        Frame* tmpFrame = m_document->frame();
        tmpFrame->loader()->client()->dispatchWillApplyCSSVisitedLink(kurl);
    }
#endif
...
#if PLATFORM(MG)
        //miniGUI use RenderingMode to implement svg font for cairo implmentation
        if (useSVGZoomRules){
            fontDescription.setRenderingMode(AlternateRenderingMode);
        }
#endif
