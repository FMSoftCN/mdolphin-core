...
CachedResource::~CachedResource()
{
    ...
    ASSERT(!inCache());
    ASSERT(!m_deleted);
    ASSERT(url().isNull() || memoryCache()->resourceForURL(KURL(ParsedURLString, url(), String())) != this);
...
void CachedResource::finish()
{
    ...
    if (!memoryCache()->disabled() && memoryCache()->isDiskCache())
    {
        if (!willAddToDisk())
            return;
        if (!memoryCache()->httpCache()->addCachedResource(this))
        {
            //setInCache(false);
        }
    }
...
//set HTTP header (m_header) according to m_response
bool CachedResource::setHttpHeader()
{
    //format:
    //HTTP/1.1_200_OK\0
    //Content-type:_XXX[;_charset=XXX]\0
    //Cache-Control:_max-age=XXX\0
    //Expires:_XXX\0
    //Last-Modified:_XXX\0
    //Content-Length:_XXX\0\0
    //
    m_header = "HTTP/1.1 200 OK";
    /*
    if (m_response.httpStatusCode())
        m_header.append(UChar(m_response.httpStatusCode()));
    else
        m_header.append("200");
    m_header.append(" ");
    
    if (!m_response.httpStatusText().isEmpty())
        m_header.append(m_response.httpStatusText());
    else
        m_header.append("OK");
    */
    ...
    if (!m_response.mimeType().isEmpty())
    {
        ...
        m_header.append(m_response.mimeType());

        if (!m_response.textEncodingName().isEmpty())
        {
            m_header.append("; charset=");
            m_header.append(m_response.textEncodingName());
        }
        m_header.append('\0');
    }

    String str = m_response.httpHeaderField("Expires");
    if (!str.isEmpty())
    {
        //md_debug("Expires is not empty\n");
        m_header.append("Expires: ");
        //time_t tm = curl_getdate(str.latin1().data(), NULL);
        m_header.append(str.utf8().data());
        m_header.append('\0');
    }
    //md_debug("CachedResource::The m_header is [%c][%c][%c][%c]\n", m_header[14], m_header[15], m_header[16], m_header[17]);

    str = m_response.httpHeaderField("Cache-Control");
    if (str.contains("no-cache", false)) 
    {
        //md_debug("no-cache, no-store\n");
        m_header.append("Cache-Control: no-cache");
        m_header.append('\0');
    }

    if (str.contains("no-store", false))
    {
        m_header.append("Cache-Control: no-store");
        m_header.append('\0');
    }
    ...
    {
        //md_debug("max-age\n");
        m_header.append("Cache-Control: max-age=");
        char buf[20];
        sprintf(buf, "%ld", m_response.expirationDate());
        m_header.append(buf);
        m_header.append('\0');
    }


    str = m_response.httpHeaderField("Last-Modified");
    if (!str.isEmpty())
    {
        //md_debug("Last-Modified is not empty\n");
        m_header.append("Last-Modified: ");
        //time_t tm = curl_getdate(str.latin1().data(), NULL);
        //m_header.append(tm);
        m_header.append(str.utf8().data());
        m_header.append('\0');
    ...
    str = m_response.httpHeaderField("ETag");
    if (!str.isEmpty())
    {
        m_header.append("ETag: ");
        m_header.append(str.utf8().data());
        m_header.append('\0');
    }

    char buffer[40];
    sprintf(buffer, "Content-Length: %lld", m_response.expectedContentLength());
    
    m_header.append(buffer);
    m_header.append('\0');

    m_header.append('\0');
#if 0
    md_debug("CachedResource: The header is ");
    for(int i=0; i<m_header.length(); ++i)
    {
        md_debug("[%c]", m_header[i]);
    }
    md_debug("\n");
#endif
