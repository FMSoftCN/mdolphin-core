...
FilePath GetPrefixedName(const FilePath& path, const String& name,
                    int index)
{
    String tmp = String::format("%s%s_%03d", "old_", name.utf8().data(), index);
...
FilePath GetTempCacheName(const FilePath& path, const String& name)
{
...
void CleanupCache(const FilePath& path_, const String& name_)
{
    for (int i = 0; i < kMaxOldFolders; i++) {
        FilePath to_delete = GetPrefixedName(path_, name_, i);
        disk_cache::DeleteCache(to_delete, true);
...
//  FIXME:TO DELETE THE FILE suyong
//#endif

    CleanupCache(path, name_str);
    return true;
}
...
void SetFieldTrialInfo(int size_group) {
  ...
  std::string group1 = String::format("CacheSizeGroup_%d", size_group);
...
//add by suyong
    Backend* CreateCacheBackend(const char* full_path, bool force,
...
// ------------------------------------------------------------------------
Backend* BackendImpl:: CreateBackend(const char *full_path, bool force,
                                int max_bytes, net::CacheType type,
                                BackendFlags flags)
{
    const String f_path = full_path;
    const FilePath path(f_path);							
    BackendImpl* cache = new BackendImpl(path);
...
*///suyong
    new_eviction_  = true; //add by suyong
...
bool BackendImpl::OpenEntry(const WTF::String& key, Entry** entry)
{
    if (disabled_)
        return false;
...
bool BackendImpl::CreateEntry(const WTF::String& key, Entry** entry)
{	
//  if (disabled_ || key.empty())
    if (disabled_ || key.isEmpty())	
        return false;
...
//  size_t key1_len = sizeof(EntryStore) - offsetof(EntryStore, key);
    unsigned key1_len = sizeof(EntryStore) - offsetof(EntryStore, key);
//  if (key.size() < key1_len ||
//      key.size() > static_cast<size_t>(kMaxInternalKeyLength))
    if (key.utf8().length() < key1_len ||
            key.utf8().length() > unsigned(kMaxInternalKeyLength))      
        num_blocks = 1;
    else
//    num_blocks = static_cast<int>((key.size() - key1_len) / 256 + 2);
        num_blocks = unsigned((key.utf8().length() - key1_len) / 256 + 2); 
...
//    stats_.OnEvent(Stats::CREATE_ERROR);
  ...
  //open_entries_[entry_address.value()] = cache_entry;
    open_entries_.set(entry_address.value(), cache_entry);
...
bool BackendImpl::DoomEntry(const WTF::String& key)
{	
    if (disabled_)
        return false;
...
bool BackendImpl::clearCache()
{
	return DoomAllEntries();
}
...
void BackendImpl::GetStats(StatsItems* stats) {
  ...
  item.first = "Entries";
  item.second = String::format("%d", data_->header.num_entries);
  stats->push_back(item);

  item.first = "Pending IO";
  item.second = String::format("%d", num_pending_io_);
  stats->push_back(item);

  item.first = "Max size";
  item.second = String::format("%d", max_size_);
  stats->push_back(item);

  item.first = "Current size";
  item.second = String::format("%d", data_->header.num_bytes);
  stats->push_back(item);
...
//    NOTREACHED();
    ...
    String tmp = String::format("f_%06x", address.FileNumber());
...
//  return num_pending_io_ > 5;  //FIXME:suyong
    return false;
}
/*
std::string BackendImpl::HistogramName(const char* name, int experiment) const {
  if (!experiment)
    return String::format("DiskCache.%d.%s", cache_type_, name);
  return String::format("DiskCache.%d.%s_%d", cache_type_, name, experiment);
}
*/
...
//    LOG(WARNING) << "Messed up entry found.";
  ...
  // We only add clean entries to the map.
     if (!*dirty)
        open_entries_.set(address.value(), cache_entry);
...
//EntryImpl* BackendImpl::MatchEntry(const std::string& key, uint32 hash,//changed by suyong
EntryImpl* BackendImpl::MatchEntry(const WTF::String& key, uint32 hash,
                                   bool find_parent)
{
    Addr address(data_->table[hash & mask_]);
    // md_debug("********* find key [%s]\n", key.utf8().data());
    // md_debug("####hash%x, %s, %s, %d\n", hash, __FILE__, __FUNCTION__, __LINE__);  
    // md_debug("####hash table address %x, %s, %s, %d\n", data_->table[hash & mask_], __FILE__, __FUNCTION__, __LINE__);
    ...
    for (;;) {
      ...
      // trust it.
            ...
            if (parent_entry) {
                ...
                // md_debug("####hash table address %x, %s, %s, %d\n", data_->table[hash & mask_], __FILE__, __FUNCTION__
                 //       , __LINE__);		
                parent_entry = NULL;
            } else {
