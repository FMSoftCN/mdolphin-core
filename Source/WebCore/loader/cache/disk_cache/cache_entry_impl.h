...
class EntryImpl : public Entry, public base::RefCounted<EntryImpl> {
    ...
    virtual void Doom();
    virtual void Close();
    virtual WTF::String GetKey() const;
    virtual base::Time GetLastUsed() const;
    virtual base::Time GetLastModified() const;
    virtual unsigned GetDataSize(int index) const;
    ...
    // cache.
    bool CreateEntry(Addr node_address, const WTF::String& key, uint32 hash);

    // Returns true if this entry matches the lookup arguments.
    bool IsSameEntry(const WTF::String& key, uint32 hash);
