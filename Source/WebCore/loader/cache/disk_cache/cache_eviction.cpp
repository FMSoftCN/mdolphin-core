...
void Eviction::TrimCacheV2(bool empty)
{
    ...
    if (empty) {
        TrimDeleted(true);
    } else if (header_->lru.sizes[Rankings::DELETED] > header_->num_entries / 10) {
        // md_debug("#################   BEGIN DELETED!!!!! #############\n");
        TrimDeleted(false);
    }
...
void Eviction::TrimDeleted(bool empty)
{
    ...
    for (int i = 0; (i < 4 || empty) && next.get(); i++) {
        // md_debug("********* In TrimDeleted ***********\n");
        node.reset(next.release());
        next.reset(rankings_->GetPrev(node.get(), Rankings::DELETED));
        RemoveDeletedNode(node.get());
    }

    if (header_->lru.sizes[Rankings::DELETED] > header_->num_entries / 10)
    {
        // md_debug("still size > 1/10\n");
    }
...
bool Eviction::RemoveDeletedNode(CacheRankingsBlock* node)
{
    ...
    if (backend_->NewEntry(Addr(node->Data()->contents), &entry, &dirty)) {
        // md_debug("NewEntry failed on Trim 0x%x", node->address().value());
        return false;
    }
    ...
    entry->Doom();
    // md_debug("entry [%s] deleted\n", entry->GetKey().utf8().data());
    entry->Release();
    return true;
}
