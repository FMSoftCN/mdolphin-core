...
void CachedImage::destroyDecodedData()
{
    ...
    //fprintf(stderr, "CachedImage::destroyDecodedData 1 m_image:%p \n", m_image.get());
    bool canDeleteImage = !m_image || (m_image.get()) && (m_image->hasOneRef() && m_image->isBitmapImage());
    //fprintf(stderr, "CachedImage::destroyDecodedData 2 m_image:%p \n", m_image.get());
    if (isSafeToMakePurgeable() && canDeleteImage && !isLoading()) {
        ...
        //fprintf(stderr, "CachedImage::destroyDecodedData 3 m_image:%p \n", m_image.get());
        m_image = 0;
        setDecodedSize(0);
        //fprintf(stderr, "CachedImage::destroyDecodedData 4 m_image:%p \n", m_image.get());
        if (!MemoryCache::shouldMakeResourcePurgeableOnEviction()) {
            //fprintf(stderr, "CachedImage::destroyDecodedData 1 m_image:%p \n", m_image.get());
            makePurgeable(true);
        }
    } else if (m_image && !errorOccurred()) {
        //fprintf(stderr, "CachedImage::destroyDecodedData 5 m_image:%p \n", m_image.get());
