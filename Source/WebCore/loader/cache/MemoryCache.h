...
#include "disk_cache/HttpCache.h"
#endif
...
public:
    ...
    CachedResource* resourceForURL(const KURL& url, CachedResource::Type type,const String& charset);
...
#if PLATFORM(MG)
    void getCapacities(unsigned* minDeadBytes, unsigned* maxDeadBytes, unsigned* totalBytes);
    bool setCacheType(CacheType type, const char* path=0, unsigned size = 0);
#if ENABLE(DISK_CACHE)
    bool isDiskCache(void){ return m_cacheType == DiskCache;}
    disk_cache::HttpCache* httpCache() {return m_httpCache;}
#endif
...
private:
    ...
    bool pruneOneDeadResource(int index, unsigned int targetSize);
...
#if PLATFORM(MG) 
    //m_cacheType indicates the cache type
    CacheType m_cacheType;

#if ENABLE(DISK_CACHE)
    //HttpCache used for disk cache management
    disk_cache::HttpCache* m_httpCache;
    
    //disk cache storage path
    const char* m_cacheDir;
    
    unsigned m_diskCapacity;
