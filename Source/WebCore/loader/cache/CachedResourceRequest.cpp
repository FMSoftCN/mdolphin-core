...
#ifndef NDEBUG
#define CRR_DEBUG_OUTPUT(fmt,...) fprintf(stderr, "File:%s, Line:%d, Function:%s, "#fmt"\n", __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#else
#define CRR_DEBUG_OUTPUT(fmt,...)
#endif
...
{
    CRR_DEBUG_OUTPUT("In m_finishing:%d\n", m_finishing);
    if (m_finishing)
        return;
    ...
    if (!m_resource->errorOccurred()) {
        ...
        if (!m_resource->errorOccurred())
            m_resource->finish();
        CRR_DEBUG_OUTPUT("##No Error##");

    }
    m_cachedResourceLoader->loadDone(this);
    CRR_DEBUG_OUTPUT("##Out##");
}
...
void CachedResourceRequest::didReceiveData(SubresourceLoader* loader, const char* data, int size)
{
    ...
    //CRR_DEBUG_OUTPUT("##[%s][%s]## In");
    if (m_resource->errorOccurred()) {
        fprintf(stderr, "##[%s][%s]## errorOccurred\n", __FILE__, __FUNCTION__);
        return;
    ...
    if (m_resource->response().httpStatusCode() >= 400) {
        ...
        //CRR_DEBUG_OUTPUT("##[%s][%s]## httpStatusCode:%d", m_resource->response().httpStatusCode());
        return;
    }
    ...
    if (m_multipart) {
        ...
        //CRR_DEBUG_OUTPUT("##[%s][%s]## m_multipart");
    } else if (m_incremental)
    {
        //CRR_DEBUG_OUTPUT("##[%s][%s]## m_incremental");
        m_resource->data(loader->resourceData(), false);
    }

    //CRR_DEBUG_OUTPUT("##[%s][%s]## Out");
}
