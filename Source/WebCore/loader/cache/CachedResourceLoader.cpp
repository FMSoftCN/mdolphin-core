...
CachedResource* createResource(CachedResource::Type type, const KURL& url, const String& charset)
{
    switch (type) {
...
CachedCSSStyleSheet* CachedResourceLoader::requestUserCSSStyleSheet(const String& requestURL, const String& charset)
{
    ...
    if (CachedResource* existing = memoryCache()->resourceForURL(url,CachedResource::CSSStyleSheet,charset)) {
...
CachedResource* CachedResourceLoader::requestResource(CachedResource::Type type, const String& resourceURL, const String& charset, ResourceLoadPriority priority, bool forPreload)
{
    ...
    CachedResource* resource = memoryCache()->resourceForURL(url,type,charset);
...
{
    ASSERT(!memoryCache()->resourceForURL(url, CachedResource::CSSStyleSheet, charset));
