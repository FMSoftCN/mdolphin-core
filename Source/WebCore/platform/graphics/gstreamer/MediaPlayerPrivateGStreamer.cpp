...
typedef WTF::HashMap<void*, bool> mediaInstanceMap_t;
mediaInstanceMap_t *gMediaInstanceMap = 0;
#endif
...
#if PLATFORM(MG)
class GstCommonMessageWrapper {
    public:
        GstCommonMessageWrapper(){}
        GstCommonMessageWrapper(GstBus* bus, GstMessage* message, gpointer data)
            : m_bus(bus)
            , m_message(message)
            , m_data(data)
        {
...
class GstStatusMessageWrapper {
    public:
        GstStatusMessageWrapper(){}
        GstStatusMessageWrapper(GObject* element, GParamSpec* pspec, gpointer data)
            : m_element(element)
            , m_paramSpec(pspec)
            , m_data(data)
        {
...
class GstRepaintMessageWrapper {
    public:
        GstRepaintMessageWrapper(){}
        GstRepaintMessageWrapper(WebKitVideoSink* sink, GstBuffer *buffer, MediaPlayerPrivateGStreamer* player)
            : m_sink(sink)
            , m_buffer(buffer)
            , m_player(player)
        {
        ...
        WebKitVideoSink* m_sink;
        GstBuffer *m_buffer;
        MediaPlayerPrivateGStreamer* m_player;
};

class GstVideoTagsMessageWrapper {
    public:
        GstVideoTagsMessageWrapper(){}
        GstVideoTagsMessageWrapper(GObject* element, gint id, MediaPlayerPrivateGStreamer* player)
            : m_element(element)
            , m_streamId(id)
            , m_player(player)
        {
...
void handleGstMessage(void *data)
{
    GstCommonMessageWrapper *object = reinterpret_cast<GstCommonMessageWrapper*>(data);
    // detect whether media player has deconstruct ,if is ,we can't call the callback
    if(gMediaInstanceMap->get(object->m_data))
        mediaPlayerPrivateMessageCallback(object->m_bus, object->m_message, object->m_data);

    gst_message_unref(object->m_message);
    delete object;
}

gboolean mediaPlayerPrivateMessageCallbackMg(GstBus* bus, GstMessage* message, gpointer data)
{
    GstCommonMessageWrapper* object = new GstCommonMessageWrapper(bus, message, data);
    gst_message_ref(message);
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&handleGstMessage, (LPARAM)object);
    return true;
}

void handleGstRepaintMessage(void *data)
{
    GstRepaintMessageWrapper *object = reinterpret_cast<GstRepaintMessageWrapper*>(data);
    if(gMediaInstanceMap->get(object->m_player))
        mediaPlayerPrivateRepaintCallback(object->m_sink, object->m_buffer, object->m_player);
    gst_buffer_unref(object->m_buffer);  
    delete object;
}

void mediaPlayerPrivateRepaintCallbackMg(WebKitVideoSink* sink, GstBuffer *buffer, MediaPlayerPrivateGStreamer* playerPrivate)
{
    GstRepaintMessageWrapper *object = new GstRepaintMessageWrapper(sink, buffer, playerPrivate);
    gst_buffer_ref(buffer);
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&handleGstRepaintMessage, (LPARAM)object);
}

void handleGstStatusMessage(void *data)
{
    GstStatusMessageWrapper *object = reinterpret_cast<GstStatusMessageWrapper*>(data);
    if(gMediaInstanceMap->get(object->m_data)) {
        if(strcmp(object->m_paramSpec->name, "source") == 0)
            mediaPlayerPrivateSourceChangedCallback(object->m_element, object->m_paramSpec, object->m_data);
        else if(strcmp(object->m_paramSpec->name, "mute") == 0)
            mediaPlayerPrivateMuteChangedCallback(object->m_element, object->m_paramSpec, object->m_data);
        else if(strcmp(object->m_paramSpec->name, "volume") == 0)
            mediaPlayerPrivateVolumeChangedCallback(object->m_element, object->m_paramSpec, object->m_data);
    }
    delete object;
}

void mediaPlayerPrivateStatusChangedCallbackMg(GObject *element, GParamSpec *pspec, gpointer data)
{
    GstStatusMessageWrapper *object = new GstStatusMessageWrapper(element, pspec, data);
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&handleGstStatusMessage, (LPARAM)object);
}

void handleGstVideoTagsMessage(void *data)
{
    GstVideoTagsMessageWrapper *object = reinterpret_cast<GstVideoTagsMessageWrapper*>(data);
    mediaPlayerPrivateVideoTagsChangedCallback(object->m_element, object->m_streamId, object->m_player);
}

void mediaPlayerPrivateVideoTagsChangedCallbackMg(GObject* element, gint streamId, MediaPlayerPrivateGStreamer* player)
{
    GstVideoTagsMessageWrapper *object = new GstVideoTagsMessageWrapper(element, streamId, player);
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&handleGstVideoTagsMessage, (LPARAM)object);
}

void mplayerAudioTagsCallback(void *data)
{
    GstVideoTagsMessageWrapper *object = reinterpret_cast<GstVideoTagsMessageWrapper*>(data);
    mediaPlayerPrivateAudioTagsChangedCallback(object->m_element, object->m_streamId, object->m_player);
}

void mediaPlayerPrivateAudioTagsChangedCallbackMg(GObject* element, gint streamId, MediaPlayerPrivateGStreamer* player)
{
    GstVideoTagsMessageWrapper *object = new GstVideoTagsMessageWrapper(element, streamId, player);
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&mplayerAudioTagsCallback, (LPARAM)object);
}
#endif
...
#if PLATFORM(MG)
    //set the old m_source hashmap false 
    if(mp->m_source)
        gMediaInstanceMap->set(mp->m_source, false);
#endif
...
#if PLATFORM(MG)
    //set the new m_source hashmap true 
    gMediaInstanceMap->set(mp->m_source, true);
#endif
...
#if PLATFORM(MG)
    if(!gMediaInstanceMap)
        gMediaInstanceMap = new mediaInstanceMap_t();
    gMediaInstanceMap->set(this, true);
#endif
...
{
#if PLATFORM(MG)
    gMediaInstanceMap->set(m_source, false);
    gMediaInstanceMap->set(this, false);

    --glibThreadRef;
    if(glibThreadRef == 0){
        g_main_loop_quit(loop);
        pthread_join(glibThreadPid, NULL);
    }
#endif
...
#if PLATFORM(MG)
    mediaPlayerPrivateVideoTagsChangeTimeoutCallback(this);
#else
    if (m_videoTagsTimerHandler)
        g_source_remove(m_videoTagsTimerHandler);
...
#if PLATFORM(MG)
    mediaPlayerPrivateAudioTagsChangeTimeoutCallback(this);
#else
    if (m_audioTagsTimerHandler)
        g_source_remove(m_audioTagsTimerHandler);
...
{
#if PLATFORM(MG)
    mediaPlayerPrivateVolumeChangeTimeoutCallback(this);
#else
    if (m_volumeTimerHandler)
        g_source_remove(m_volumeTimerHandler);
...
{
#if PLATFORM(MG)
    mediaPlayerPrivateMuteChangeTimeoutCallback(this);
#else
    if (m_muteTimerHandler)
        g_source_remove(m_muteTimerHandler);
...
#if PLATFORM(MG)
void* g_main_loop_run_thread(void *data)
{
...
int MediaPlayerPrivateGStreamer::glibThreadRef = 0;
pthread_t MediaPlayerPrivateGStreamer::glibThreadPid = 0;
#endif
...
#if PLATFORM(MG)
    g_signal_connect(bus, "message", G_CALLBACK(mediaPlayerPrivateMessageCallbackMg), this);
#else
...
#if PLATFORM(MG)
    hWndMedia = (static_cast<HTMLMediaElement *>(m_player->mediaPlayerClient()))->document()->frame()->page()->chrome()->platformPageClient();
    if(glibThreadRef == 0)
        pthread_create(&glibThreadPid, NULL, g_main_loop_run_thread, NULL);
    ++glibThreadRef;

    g_signal_connect(m_playBin, "notify::volume", G_CALLBACK(mediaPlayerPrivateStatusChangedCallbackMg), this);
    g_signal_connect(m_playBin, "notify::source", G_CALLBACK(mediaPlayerPrivateStatusChangedCallbackMg), this);
    g_signal_connect(m_playBin, "notify::mute", G_CALLBACK(mediaPlayerPrivateStatusChangedCallbackMg), this);
    //FIXME: Check if the following is right here.
    g_signal_connect(m_playBin, "video-tags-changed", G_CALLBACK(mediaPlayerPrivateVideoTagsChangedCallbackMg), this);
    g_signal_connect(m_playBin, "audio-tags-changed", G_CALLBACK(mediaPlayerPrivateAudioTagsChangedCallbackMg), this);
#else
...
#if PLATFORM(MG)
    g_signal_connect(m_webkitVideoSink, "repaint-requested", G_CALLBACK(mediaPlayerPrivateRepaintCallbackMg), this);
#else
