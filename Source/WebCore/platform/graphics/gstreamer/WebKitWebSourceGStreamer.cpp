...
#if PLATFORM(MG)
#include "minigui.h"
extern HWND hWndMedia;

static gboolean webKitWebSrcSeekMainCbMG(void* argv);
static gboolean webKitWebSrcEnoughDataMainCbMG(void* argv);
static gboolean webKitWebSrcNeedDataMainCbMG(void* argv);
#endif
...
{
#if PLATFORM(MG)
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&webKitWebSrcNeedDataMainCbMG,(LPARAM)src);
    return FALSE;
#else
    WebKitWebSrcPrivate* priv = src->priv;
...
static gboolean webKitWebSrcNeedDataMainCbMG(void* argv)
{
    WebKitWebSrc* src=(WebKitWebSrc* )argv; 
    WebKitWebSrcPrivate* priv = src->priv;
...
{
#if PLATFORM(MG)
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&webKitWebSrcEnoughDataMainCbMG,(LPARAM)src);
    return FALSE;
#else
    WebKitWebSrcPrivate* priv = src->priv;
...
{
#if PLATFORM(MG)
    SendNotifyMessage(hWndMedia, MSG_GSTREAMER, (WPARAM)&webKitWebSrcSeekMainCbMG,(LPARAM)src);
    return FALSE;
#else
...
extern mediaInstanceMap_t *gMediaInstanceMap;
static gboolean webKitWebSrcSeekMainCbMG(void* argv)
{
    WebKitWebSrc* src=(WebKitWebSrc* )argv; 
    if(gMediaInstanceMap->get(src)){
        webKitWebSrcStop(src, true);
        webKitWebSrcStart(src);
    }
    return FALSE;
