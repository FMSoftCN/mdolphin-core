...
#if PLATFORM(MG)
    GraphicsContext* oldContext = paintInfo.context;
    bool isCairoContext = oldContext->isCairoCanvas();
    if (!isCairoContext) {
        oldContext->save();
        paintInfo.context = oldContext->newCairoContext(oldContext);
    }
#endif
...
#if PLATFORM(MG)
    if (!isCairoContext) {
        oldContext->deleteCairoContext(static_cast<GraphicsContextCairo*>(paintInfo.context));
        oldContext->restore();
        paintInfo.context = oldContext;
    }
#endif
}
