...
public:
    JSElementConstructor(JSC::ExecState*, JSDOMGlobalObject*);
...
JSElementConstructor::JSElementConstructor(ExecState* exec, JSDOMGlobalObject* globalObject)
    : DOMConstructorObject(JSElementConstructor::createStructure(globalObject->globalData(), globalObject->objectPrototype()), globalObject)
{
...
static const HashTableValue JSElementPrototypeTableValues[30] =
{
    ...
    { "webkitGetAnimations", DontDelete | Function, (intptr_t)static_cast<NativeFunction>(jsElementPrototypeFunctionWebkitGetAnimations), (intptr_t)0 THUNK_GENERATOR(0) },
...
{
    return new (exec) JSElementPrototype(globalObject, JSElementPrototype::createStructure(exec->globalData(), JSNodePrototype::self(exec, globalObject)));
}
...
EncodedJSValue JSC_HOST_CALL jsElementPrototypeFunctionWebkitGetAnimations(ExecState* exec)
{
    JSValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(&JSElement::s_info))
        return throwVMTypeError(exec);
    ...
    JSC::JSValue result = toJS(exec, castedThis->globalObject(), WTF::getPtr(imp->webkitGetAnimations()));
    return JSValue::encode(result);
}
